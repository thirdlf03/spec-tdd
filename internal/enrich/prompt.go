...
セグメントが functional_requirement または non_functional_requirement の場合、仕様から Given/When/Then 形式の Examples を**網羅的に**生成してください。
...
以下のカテゴリを**すべて確認**し、該当する仕様があれば必ず Example を生成してください。

### A. エンドポイント別
**POST**: 最小フィールド作成 / 全フィールド作成 / ID重複(409) / 一意性違反(409) / 各バリデーション(422) / unknown field(400) / 不正JSON(400) / Content-Type不正(415)
**GET単体**: 正常(200+ETag) / 存在しないID(404) / 論理削除済み(404) / includeDeleted=true(200)
**GETリスト（重要）**: フィルタなし / 各クエリパラメータ(q,status,tags,sort,order,limit,offset,includeDeleted等) / sortの各値を個別テスト / boolean/enumフィルタの全有効値テスト / 不正パラメータ値(422) / ページネーション境界値(limitの最小・最大) / ソート順序+null値の扱い
**PATCH**: 正常更新 / nullクリア / 空オブジェクト(400) / 不変フィールドを各個テスト(400) / 存在しない(404) / 削除済み(409) / 一意性違反(409) / If-Match欠落(428) / If-Match不一致(412) / POSTと同じバリデーションをPATCH時にもテスト(文字数上限,フォーマット,範囲外等) / PATCH時の正規化テスト
**DELETE**: 正常削除 / 既に削除済み(404) / 存在しない(404) / If-Match欠落(428) / If-Match不一致(412) / 依存制約

### B. 境界値テスト
文字数上限・下限がある場合: 上限ちょうど(成功) + 上限+1(失敗) / 下限ちょうど(成功) + 下限-1(失敗)
配列要素数上限: 上限ちょうど(成功) + 上限+1(失敗)
数値範囲: 下限-1 + 上限+1

### C. 共通仕様の適用
共通仕様のルールは各リソースに対してテストを生成: ETag/If-Match(428/412) / 正規化(trim,dedup,sort等) / エラー形式(error.code)

### D. データ正規化・状態遷移
Unicode正規化 / 空白正規化 / null変換 / 配列正規化 / 状態遷移制約 / 依存関係制約 / カスケード動作
